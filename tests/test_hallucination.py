...
class TestNLIHallucinationDetectorDetect:
    ...
    def test_clear_entailment_not_hallucination(self) -> None:
        ...
        result = detector.detect(
            ground_truth="Aspirin is an NSAID.",
            model_prediction="Aspirin reduces pain and fever.",
        )
        assert result.is_hallucination is False
    ...
    def test_clear_contradiction_is_hallucination(self) -> None:
        ...
        result = detector.detect(
            ground_truth="Aspirin is an NSAID used to reduce fever.",
            model_prediction="Aspirin is an ACE inhibitor used to treat hypertension.",
        )
    ...
    def test_high_neutral_is_hallucination(self) -> None:
        ...
        result = detector.detect(
            ground_truth="Fever is managed with paracetamol.",
            model_prediction="The patient should fast for 24 hours.",
        )
    ...
    def test_exact_threshold_boundary_is_not_hallucination(self) -> None:
        ...
        result = detector.detect(ground_truth="Context text.", model_prediction="Claim text.")
    ...
    def test_result_contains_raw_output(self) -> None:
        ...
        result = detector.detect(ground_truth="Premise.", model_prediction="Hypothesis.")
        assert "raw_list" in result.raw_output

    def test_detect_empty_inputs_raise(self) -> None:
        """Empty strings for ground_truth or model_prediction must raise ValueError."""
        detector = NLIHallucinationDetector()
        with pytest.raises(ValueError, match="ground_truth"):
            detector.detect(ground_truth="", model_prediction="Valid.")
        with pytest.raises(ValueError, match="model_prediction"):
            detector.detect(ground_truth="Valid.", model_prediction="   ")
    ...
    def test_raises_import_error_when_transformers_missing(self) -> None:
        ...
        with patch.dict("sys.modules", {"transformers": None}):
            with pytest.raises((ImportError, TypeError)):
                detector.detect(ground_truth="Context.", model_prediction="Claim.")
...
class TestNLIHallucinationDetectorBatch:
    """Tests for the detect_batch method of NLIHallucinationDetector."""
    ...
    def test_batch_returns_correct_number_of_results(self) -> None:
        ...
        results = detector.detect_batch(
            ground_truths=["Context A.", "Context B.", "Context C."],
            model_predictions=["Claim A.", "Claim B.", "Claim C."],
        )

        assert len(results) == 3
        assert all(isinstance(r, NLIResult) for r in results)
        assert detector._pipeline.call_count == 3  # Called once per item
    ...
    def test_batch_raises_on_empty_lists(self) -> None:
        ...
        with pytest.raises(ValueError, match="non-empty"):
            detector.detect_batch(ground_truths=[], model_predictions=[])
